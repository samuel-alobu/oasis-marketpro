...
export default function DashboardPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const holdingSymbols = new Set(
            holdingsWithBalance.map((h: Holding) => h.symbol),
          );
          ...
          setHoldings(
            mergedHoldings.length > 0 ? mergedHoldings : defaultAssets,
          );
          ...
          if (totalDeposits > 0) {
            setTradingProgress(
              Math.min(100, Math.round((totalDeposits / 10000) * 100)),
            );
            setSignalStrength(
              Math.min(100, Math.round((totalDeposits / 5000) * 100)),
            );
          }
        }
      } catch (error: any) {
      ...
      const interval = setInterval(() => {
        setPositions((prev) =>
          prev.map((p) => {
            ...
            if (p.stopLoss) {
              if (
                (p.type === "buy" && newPrice <= p.stopLoss) ||
                (p.type === "sell" && newPrice >= p.stopLoss)
              ) {
                shouldClose = true;
            ...
            if (p.takeProfit) {
              if (
                (p.type === "buy" && newPrice >= p.takeProfit) ||
                (p.type === "sell" && newPrice <= p.takeProfit)
              ) {
                shouldClose = true;
  ...
  const handleExecuteTrade = async () => {
    ...
    if (marginRequired > tradingAccount.balance) {
      addToast(
        "error",
        `${t.dashboard.marginRequired} ${formatCurrency(marginRequired)}`,
      );
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setPositions((prev) => [
          {
            ...data.position,
            expiresAt: data.position.expiresAt
              ? new Date(data.position.expiresAt)
              : undefined,
          },
          ...prev,
        ]);
        ...
        if (data.account) {
          setTradingAccount((prev) =>
            prev ? { ...prev, balance: data.account.balance } : prev,
          );
        }

        addToast(
          "success",
          `${orderTab.toUpperCase()} ${t.dashboard.tradeExecuted} ${tradeAmount} ${selectedAsset.symbol}`,
        );
      } else {
  ...
  const handleClosePositionAPI = async (
    positionId: string,
    currentPrice?: number,
    ...
    try {
      ...
      if (res.ok) {
        ...
        addToast(
          data.position.pnl >= 0 ? "success" : "error",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="lg:col-span-2 bg-[#0f1419] rounded-xl border border-[#1e2733] p-4 md:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 md:mb-6">
            ...
            <Link
              href="/deposit"
              className="hidden sm:flex items-center justify-center gap-2 px-4 py-2 bg-[#22c55e] hover:bg-[#1ea550] text-white text-sm font-medium rounded-lg transition-colors"
            >
          ...
          <div className="grid grid-cols-4 gap-2 mb-6 md:hidden">
            <Link
              href="/deposit"
              className="flex flex-col items-center gap-2 p-3 bg-[#151c24] border border-[#1e2733] rounded-lg text-white hover:border-[#22c55e] transition-colors"
            >
          ...
          <div className="hidden md:flex flex-wrap gap-2 md:gap-3 mb-6 md:mb-8">
            <Link
              href="/deposit"
              className="flex items-center gap-2 px-3 md:px-4 py-2 md:py-2.5 bg-[#151c24] border border-[#1e2733] rounded-lg text-white hover:border-[#22c55e] transition-colors text-sm"
            >
                ...
                {isLoading ? (
                  <div className="space-y-2">
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between p-2 bg-[#151c24] rounded-lg"
                      >
                  ...
                  <div className="space-y-2">
                    {holdings.slice(0, 6).map((asset) => (
                      <div
                        key={asset.symbol}
                        className="flex items-center justify-between p-2 bg-[#151c24] rounded-lg hover:bg-[#1a2129] transition-colors"
                      >
                        <div className="flex items-center gap-2">
                          {asset.icon ? (
                            <Image
                            ...
                            <div
                              ...
                              style={{
                                backgroundColor: asset.color || "#555555",
                              }}
                            >
                        ...
                        <div className="text-right">
                          <p className="text-white font-medium text-sm">
                            {formatCurrency(asset.amountUsd)}
                          </p>
                          <p className="text-[#6b7a90] text-xs">
                ...
                {isLoading ? (
                  <div className="space-y-2">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="h-14 bg-[#1e2733] rounded-lg animate-pulse"
                      />
                    ))}
                  </div>
                ) : !hasActivities ? (
                  <div className="text-center py-8">
                    <p className="text-[#6b7a90] text-sm mb-2">
                      {t.dashboard.recentActivity}
                    </p>
                    <Link
                      href="/deposit"
                  ...
                  <div className="space-y-2">
                    {recentActivities.slice(0, 6).map((activity) => (
                      <div
                        key={activity.id}
                        className="flex items-center gap-3 p-3 bg-[#151c24] rounded-lg"
                      >
                        <div
                          className={cn(
                        ...
                        >
                          {activity.type === "deposit" ? (
                            <ArrowDown className="h-4 w-4 text-[#22c55e]" />
                          ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[#6b7a90] text-xs truncate">
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <p className="text-xs text-[#6b7a90] capitalize">
                            {activity.status}
                          </p>
                        </div>
                      </div>
          ...
          <div className="hidden md:block">
            <div className="flex items-center justify-between mb-3 md:mb-4">
              ...
              <Link
                href="/assets"
                className="text-[#3b82f6] text-xs md:text-sm hover:underline flex items-center gap-1"
              >
                {t.common.all}{" "}
            ...
            {isLoading ? (
              <div className="hidden md:block space-y-2 md:space-y-3">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div
                    key={i}
                    className="flex items-center justify-between p-2 md:p-3 bg-[#151c24] rounded-lg"
                  >
              ...
              <div className="hidden md:block space-y-2 md:space-y-3">
                {holdings.slice(0, 6).map((asset) => (
                  <div
                    key={asset.symbol}
                    className="flex items-center justify-between p-2 md:p-3 bg-[#151c24] rounded-lg hover:bg-[#1a2129] transition-colors"
                  >
                    <div className="flex items-center gap-2 md:gap-3">
                      {asset.icon ? (
                        <Image
                        ...
                        <div
                          ...
                          style={{ backgroundColor: asset.color || "#555555" }}
                        >
                          {asset.symbol.charAt(0)}
                        </div>
                      )}
                      <div>
                        ...
                        <p className="text-[#6b7a90] text-xs md:text-sm">
                          {asset.symbol}
                        </p>
                      </div>
                    </div>
        ...
        <div className="bg-[#0f1419] rounded-xl border border-[#1e2733] p-4 md:p-6">
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-2 md:gap-3">
                  <div className="bg-[#151c24] rounded-lg p-2 md:p-3">
                    <p className="text-[#6b7a90] text-xs mb-0.5">
                      {t.dashboard.totalPnl}
                    </p>
                    <p
                      className={cn(
                        "text-sm md:text-base font-semibold",
                        (tradingAccount?.totalPnl || 0) >= 0
                          ? "text-[#22c55e]"
                          : "text-[#ef4444]",
                      )}
                    >
                      {(tradingAccount?.totalPnl || 0) >= 0 ? "+" : ""}
                      {formatCurrency(tradingAccount?.totalPnl || 0)}
                    </p>
                  </div>
                  <div className="bg-[#151c24] rounded-lg p-2 md:p-3">
                    <p className="text-[#6b7a90] text-xs mb-0.5">
          ...
          <h3 className="text-white font-medium mb-3 md:mb-4 text-sm md:text-base">
            {t.dashboard.recentActivity}
          </h3>
          ...
          {isLoading ? (
            <div className="space-y-2 md:space-y-3">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-14 md:h-16 bg-[#1e2733] rounded-lg animate-pulse"
                />
              ))}
            </div>
          ) : !hasActivities ? (
            ...
            <div className="space-y-2 md:space-y-3">
              {recentActivities.slice(0, 4).map((activity) => (
                <div
                  key={activity.id}
                  className="flex items-center gap-2 md:gap-3 p-2 md:p-3 bg-[#151c24] rounded-lg"
                >
                  <div
                    className={cn(
                  ...
                  >
                    {activity.type === "deposit" ? (
                      <ArrowDown className="h-3 w-3 md:h-4 md:w-4 text-[#22c55e]" />
                    ) : (
                      <ArrowUp className="h-3 w-3 md:h-4 md:w-4 text-[#ef4444]" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
          ...
          <div className="mt-4 md:mt-6 space-y-3 md:space-y-4">
            <div className="bg-[#151c24] rounded-lg p-3 md:p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[#6b7a90] text-xs md:text-sm">
                  {t.dashboard.tradingProgress}
                </span>
                <span className="text-[#22c55e] font-medium text-xs md:text-sm">
                  {tradingProgress}%
                </span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="bg-[#0f1419] rounded-xl border border-[#1e2733] p-3 md:p-4 overflow-hidden md:h-[500px] md:overflow-y-auto">
          ...
          <div className="flex items-center justify-between mb-3 md:mb-4 pb-3 border-b border-[#1e2733]">
            ...
            <Link
              href="/trade"
              className="flex items-center gap-1 px-2 md:px-3 py-1.5 bg-[#22c55e] hover:bg-[#1ea550] text-white text-xs font-medium rounded-lg transition-colors"
            >
          ...
          <div className="flex gap-1 md:gap-2 mb-3 md:mb-4">
            {(["buy", "sell", "convert"] as OrderTab[]).map((tab) => (
              <button
                ...
                className={cn(
                  "flex-1 py-2 md:py-2.5 rounded-lg text-xs md:text-sm font-medium transition-colors capitalize",
                  orderTab === tab
                    ? tab === "buy"
                      ? "bg-[#22c55e] text-white"
                      : tab === "sell"
                        ? "bg-[#ef4444] text-white"
                        : "bg-[#6b7a90] text-white"
                    : "bg-[#151c24] text-[#6b7a90] hover:text-white",
                )}
              >
                {tab === "buy"
                  ? t.trading.buy
          ...
          {/* Trade Type */}
          <div className="mb-3">
            <label className="text-xs text-[#6b7a90] mb-1 block">
              {t.dashboard.type}:
            </label>
            <div className="relative">
              <button
                onClick={() => setIsTypeDropdownOpen(!isTypeDropdownOpen)}
                className="w-full flex items-center justify-between px-2 md:px-3 py-2 bg-[#0a0e14] border border-[#1e2733] rounded-lg text-white text-sm"
              >
                <span>{tradeType}</span>
                <ChevronDown
                  className={cn(
                    "h-3 w-3 md:h-4 md:w-4 text-[#6b7a90] transition-transform",
                  ...
                  >
                    {["Crypto", "Stocks"].map((type) => (
                      <button
                        key={type}
                        onClick={() => {
                          setTradeType(type as "Crypto" | "Stocks");
                          const firstAsset = tradingAssets.find(
                            (a) => a.type === type,
                          );
          ...
          <div className="mb-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 min-w-0 px-2 md:px-3 py-2 bg-[#0a0e14] border border-[#1e2733] rounded-lg text-white text-sm focus:outline-none focus:border-[#22c55e]"
                placeholder="0"
              />
              <div className="relative shrink-0">
                <button
                  onClick={() => setIsAssetDropdownOpen(!isAssetDropdownOpen)}
                  className="flex items-center gap-1 px-2 py-2 bg-[#0a0e14] border border-[#1e2733] rounded-lg text-white whitespace-nowrap"
                >
                  ...
                  <ChevronDown
                    className={cn(
                      "h-3 w-3 text-[#6b7a90] transition-transform",
                ...
                <AnimatePresence>
                  {isAssetDropdownOpen && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className="absolute top-full right-0 mt-1 w-32 md:w-40 bg-[#0f1419] border border-[#1e2733] rounded-lg shadow-xl z-20 max-h-40 md:max-h-48 overflow-y-auto"
                    >
                      {filteredAssets.map((asset) => (
                        <button
                          ...
                          className={cn(
                            "w-full px-3 py-2 text-left text-xs md:text-sm transition-colors",
          ...
          <div className="mb-3">
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-[#6b7a90]">
                {t.trading.leverage}:
              </label>
          ...
          <div className="space-y-1 mb-3 text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white">
                {formatCurrency(
                  (parseFloat(amount) * selectedAsset.price) / leverage || 0,
                )}
              </span>
            </div>
          </div>
          ...
          {useTpSl && (
            <div className="grid grid-cols-2 gap-2 mb-3">
              <div>
                <label className="text-[10px] text-[#6b7a90] mb-0.5 block">
                  {t.trading.stopLoss}:
                </label>
                <input
                  type="number"
                  ...
                  className="w-full px-2 py-1.5 bg-[#0a0e14] border border-[#1e2733] rounded-lg text-white text-xs focus:outline-none focus:border-[#22c55e]"
                />
              </div>
              <div>
                <label className="text-[10px] text-[#6b7a90] mb-0.5 block">
              ...
              <AnimatePresence>
                {isDurationDropdownOpen && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -10 }}
                    className="absolute bottom-full left-0 right-0 mb-1 bg-[#0f1419] border border-[#1e2733] rounded-lg shadow-xl z-20 max-h-40 overflow-y-auto"
                  >
      ...
      <div className="bg-[#0f1419] rounded-xl border border-[#1e2733] p-4 md:p-6">
        ...
        <div className="mb-3 md:mb-4">
          <button
            ...
            className="flex items-center gap-2 text-[#6b7a90] hover:text-white mb-2 text-sm"
          >
            <Circle className="h-3 w-3 md:h-4 md:w-4 text-[#22c55e]" />
            <span>
              {t.common.open} ({openPositions.length})
            </span>
            {openTradesExpanded ? (
            ...
            {openTradesExpanded && (
              ...
              >
                {openPositions.length === 0 ? (
                  <div className="text-[#6b7a90] text-xs md:text-sm py-4 text-center bg-[#151c24] rounded-lg">
                    {t.dashboard.noOpenTrades}
                  </div>
                ) : (
                  <div className="space-y-2">
                      ...
                      >
                        <div className="flex items-center gap-2 md:gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] md:text-xs text-[#6b7a90]">
                        ...
                        <div className="flex items-center gap-2 md:gap-4">
                          ...
                          <button
                            ...
                            className="px-2 md:px-3 py-1 bg-[#ef4444]/10 text-[#ef4444] rounded text-[10px] md:text-xs hover:bg-[#ef4444]/20 transition-colors"
                          >
                            {t.common.close}
                          </button>
                        </div>
        ...
        <div>
          ...
          >
            ...
            <span>
              {t.dashboard.closed} ({closedPositions.length})
            </span>
                ...
                {closedPositions.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {closedPositions.slice(0, 5).map((pos) => (
                      <div
                        key={pos.id}
                        className="flex items-center justify-between p-2 md:p-3 bg-[#151c24] rounded-lg opacity-70"
                      >
                        <div className="flex items-center gap-2 md:gap-3">
                          <div
                            className={cn(
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[10px] md:text-xs text-[#6b7a90] capitalize">
        ...
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              ...
              className={cn(
                "flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2 md:py-3 rounded-xl shadow-xl min-w-[200px] md:min-w-[280px]",
